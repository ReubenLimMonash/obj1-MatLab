% Date: 21/12/2022
% To test sum of generalised-K as a gamma
...
m = mean(gen_K); std_dev = std(gen_K); var = std_dev^2;
k = m^2/var;
theta = var/m;
...
h = histogram(gen_K,'BinWidth',bin_width,'Normalization','pdf'); hold on;
...
plot(c,gamma_PDF,'r','LineWidth',1.5)
% lognormal_PDF = log_normal_pdf(c,mu_ln,sigma_ln);
% plot(c,lognormal_PDF,'m','LineWidth',1)
xlabel("X"); ylabel("f(X)");
% title(sprintf("K = %d dB",K_dB));
legend("Simulated sum of generalised-K","Gamma approx");

figure();
num_points = 150;
c = linspace(min(gen_K),pdf_range,num_points+1);
NCS_cdf = zeros(1,num_points);
for i = 1:num_points
    NCS_cdf(i) = sum(gen_K<=c(i+1))/N;
end
c_points = c(2:end);
...
L_NCS_pdf = h.Values; %PDF values
c_L_NCS = conv(h.BinEdges, [0.5 0.5], 'valid'); %PDF points
h2 = histogram(gen_K,'BinWidth',bin_width,'Normalization','pdf'); hold on;
gen_K_pdf = h2.Values; %PDF values
c_gen_K = conv(h2.BinEdges, [0.5 0.5], 'valid'); %PDF points

figure();
plot(c_L_NCS, L_NCS_pdf); hold on;
plot(c_gen_K, gen_K_pdf); hold on;
xlabel("X"); ylabel("f(X)");
legend("Sum of interferers as L-NCS","Sum of generalized-K");

figure();
num_points = 150;
c = linspace(min(gen_K),pdf_range,num_points+1);
L_NCS_cdf = zeros(1,num_points);
gen_K_cdf = zeros(1,num_points);
for i = 1:num_points
    L_NCS_cdf(i) = sum(L_NCS <= c(i+1))/N;
end
c_points = c(2:end);
index = 1;
for i = c_points
    gen_K_cdf(index) = sum(gen_K <= i)/N;
    index = index + 1;
end
plot(c_points,L_NCS_cdf,'LineWidth',1.5); hold on;
plot(c_points,gen_K_cdf,'LineWidth',1.5); hold on;
xlabel("x"); ylabel("F(x)");
legend("Sum of interferers as L-NCS","Sum of generalized-K");
