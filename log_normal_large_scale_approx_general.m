function [mu_psi_j, sigma_psi_j] = log_normal_large_scale_approx_general(hL, hi, dL, di, K, omega, sigma_N, noise_dB, n)
%For either U2G analysis or U2U analysis
%Date: 17/10/2022
%Desc: Moment matching approach to approximate interference and large-scale
%      power received as lognormal distribution.

%hL = Deterministic large-scale path loss of SOI, 
%hi = Deterministic large-scale path loss of interferers (vector), 
%dL = distance of SOI
%di = vector of distances for interferers
...
assert((length(hi) == num_I), "Interferer Tx power vector mismatch"); %Check hi vector
...
for i = 1:num_I %For each distance in interferer list
    mu_psi = mu_N/eta + 10*log10(hi(i)*di(i)^(-n(i+1))); %Logarithmic mean of large-scale power from ith interferer
