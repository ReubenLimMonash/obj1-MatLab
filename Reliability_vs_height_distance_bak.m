...
for j = 1:length(height)
    ...
    for k = 1:length(dL_H)
        ...
        reliability_sim(j,k) = 1 - interp1(c,CDF_sinr_sim,sinr_th,'spline');
        %======================================================================
...
assert(sum(sum(isnan(reliability_sim)))==0, "NaN exits in reliability_sim");
...
%%
% Save the variable
Note = "Antenna gains for U2G set to 1. R = 5Mbps. 7 member UAVs";
save("Reliability_vs_Height_Dist_Pi10dBm_R5e6_7UAV.mat",'dL_H','di','Pt','Pi','height','theta_GCS',...
