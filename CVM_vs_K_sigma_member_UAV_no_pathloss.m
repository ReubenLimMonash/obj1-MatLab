...
for k = 1:length(K_L_dB)
    for j = 1:length(sigma_N_L_dB)
        ...
        Pwr_sim = ricePwrrnd(v_i*ones(1, N),s_i).*10.^(normrnd(0,sigma_N_i,1,N)./10);
        I_sim = ones(length(di)+1, N);
        if ~isempty(di)
            for i = 1:length(di)
                % Simulating interference power from other member UAVs to
                % leader UAV
                I_sim(i,:) = ricePwrrnd(v_i*ones(1, N),s_i).*10.^(normrnd(0,sigma_N_i,1,N)./10);
            end
        end
        % Simulating interference power from GCS to leader UAV
        I_sim(length(di)+1,:) = ricePwrrnd(v_L*ones(1, N),s_L).*10.^(normrnd(0,sigma_N_L,1,N)./10);
        if ~isempty(di)
            I_sim = sum(I_sim);
        end
...
%         h = histogram(SINR_sim,300,'BinLimits',[0,max(SINR_sim)],'Normalization','cdf');
        ...
        n = [0, zeros(1,length(di)), 0]; %Corresponding to [GCS, Member 1, Member 2]
        hI = [ones(1,length(di)), 1];
        % Get the lognormal approximation CDF
        [mu_z_0,sigma_z_0] = log_normal_large_scale_approx_general(1, hI, dm, [di, dL], K(2:end), omega, sigma_N, noise_dB, n);
        ...
        [k1,k2,theta1,theta2] = beta_prime_approx_v2_general(1, hI, dm, [di, dL], K, omega, sigma_N, noise_dB, n);
...
save("CVM_vs_K_sigma_3_member_UAV_no_pathloss.mat",'CVM_ln','CVM_beta','K_L_dB','sigma_N_L_dB',...
    'dL_H','di','dm','Pt','Pi','height','freq','noise','n');
%%
%Save results to file (for mean of abs error)
save("MAE_vs_K_sigma_3_member_UAV_no_pathloss.mat",'MAE_ln','MAE_beta','K_L_dB','sigma_N_L_dB',...
    'dL_H','di','dm','Pt','Pi','height','freq','noise','n');
%%
%Save results to file (for mean of relative error)
save("MRE_vs_K_sigma_3_member_UAV_no_pathloss.mat",'MRE_ln','MRE_beta','K_L_dB','sigma_N_L_dB',...
    'dL_H','di','dm','Pt','Pi','height','freq','noise','n');
%%
