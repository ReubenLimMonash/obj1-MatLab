...
height = 10; %Height btw UAVs and GCS (m)
dL_H = 80; %Horizontal distance between leader UAV and GCS
...
di = ones(1,Num_member_UAV-1)*di; % Interfering UAVs distances
noise_dB = -107;
...
P_sim =  hi*dm^(-n_i).*ricePwrrnd(v_i*ones(1, N),s_i);
...
%         I_sim(i,:) = hL*dL.^(-n_L).*ricePwrrnd(v_L*ones(1, N),s_L).*10.^(normrnd(0,sigma_N_L,1,N)./10);
    end
end
...
% I_sim(length(di)+1,:) = hi*(dm).^(-n_i).*ricePwrrnd(v_i*ones(1, N),s_i).*10.^(normrnd(0,sigma_N_i,1,N)./10);
if ~isempty(di)
    I_sim = sum(I_sim);
end
SINR_sim = Pwr_sim./(I_sim+noise); %SINR
S_sim = P_sim./(I_sim+noise);
...
% K = [K_i, K_i, K_i, K_L]; %Corresponding to [GCS, Member 1, Member 2]
% sigma_N = [sigma_N_i, sigma_N_i, sigma_N_i, sigma_N_L]; %Corresponding to [GCS, Member 1, Member 2]
% n = [n_i, n_i, n_i, n_L]; %Corresponding to [GCS, Member 1, Member 2]
% hI = [hi, hi, hL]; di = [5 5]; dm = 5;
